...
const isOnline = () => {
  if (navigator.onLine) {
    ...
    toastMessage('Estas offline B)').showToast()
  }
}
...
const verifyNotifications = (activated) => {
  if (activated) {
    $('#notifiyActivated').css('display', 'block');
    $('#notifiyDeactivated').css('display', 'none');
  } else {
    $('#notifiyActivated').css('display', 'none');
    $('#notifiyDeactivated').css('display', 'block')
  }
};
...
$(document).on('click', '#notificacions', async e => {
  try {
    if (!swReg) return;
